...
Council roles belong to OMOS transactions, not permanently to model vendors. A run may assign roles such as `architect`, `builder`, `adversarial_reviewer`, `verifier`, `synthesizer`, or `domain_specialist`.

The assignment must be stored with the contribution. A provider must not be treated as permanently synonymous with one role. A provider that authored a change, conclusion, or artifact must not serve as the sole final verifier of that same work.
...
`providerRequestId` may be null where an upstream API does not expose a stable request identifier. A missing upstream identifier must never be replaced with a fabricated value.
...
- `disputed` — claimed provider attribution conflicts with available evidence or has been challenged.
...
- `LIVE` — every required contribution was captured through a live adapter.
...
A factual contradiction should become a first-class object:
...
Model agreement is comparative evidence. It is not factual verification. A deterministic rule or schema can make execution repeatable without making the underlying conclusion true.
...
The broader specification may support dispositions such as APPROVE, REJECT, MODIFY, or REQUEST_MORE_EVIDENCE. The current OMOS runtime presently supports `APPROVED` and `REJECTED`; additional server-side states must not be represented as operational until implemented and regression-tested.

A valid result may be approved with dissent, deferred, rejected, or classified as insufficient evidence. Consensus is not required for successful completion.
