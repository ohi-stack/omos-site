const { SITE, tools, documents, pluginTargets } = require('../content/site-content');

function renderDashboard() {
  const stats = [
    { label: 'Node Runtime', value: 'Active', note: '/health and /manifest available' },
    { label: 'Public Pages', value: '10+', note: 'Core OMOS content routes mapped' },
    ...
    { label: 'App Mirror', value: 'Required', note: 'Mirror status in app.OneGodian.com' }
  ];
...
<head>
  ...
  <title>OMOS Dashboard | ${SITE.shortName}</title>
  <style>
    :root{--bg:#07111f;--panel:#0d1b2a;--panel2:#11263a;--text:#eef6ff;--muted:#9fb8d1;--accent:#4fd1ff;--gold:#ffd76b;--green:#90ffcf;--radius:22px}*{box-sizing:border-box}body{margin:0;min-height:100vh;background:radial-gradient(circle at 20% 20%,rgba(79,209,255,.16),transparent 24%),linear-gradient(180deg,#06101b,#091626);color:var(--text);font-family:Inter,Segoe UI,Arial,sans-serif}.wrap{max-width:1240px;margin:0 auto;padding:26px}.top{display:flex;justify-content:space-between;gap:20px;align-items:center;border-bottom:1px solid rgba(255,255,255,.08)}a{color:inherit}.brand{font-weight:900;text-decoration:none}.btn{display:inline-flex;padding:12px 15px;border:1px solid rgba(255,255,255,.12);border-radius:999px;text-decoration:none;color:#d8f7ff}.hero{padding:52px 0 28px}h1{font-size:clamp(38px,6vw,68px);line-height:1;margin:0 0 14px;letter-spacing:-.05em}.muted{color:var(--muted);line-height:1.6;max-width:860px}.grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:18px}.card{border:1px solid rgba(255,255,255,.09);border-radius:var(--radius);background:linear-gradient(180deg,var(--panel),var(--panel2));padding:22px;min-height:160px}.card .label{font-size:12px;color:var(--gold);letter-spacing:.16em;text-transform:uppercase}.card .value{font-size:34px;font-weight:900;margin:12px 0 8px}.card p{margin:0;color:var(--muted);line-height:1.5}.checklist{display:grid;gap:12px;margin-top:18px}.check{border:1px solid rgba(255,255,255,.09);border-radius:16px;padding:14px;background:rgba(255,255,255,.035);color:#d8f7ff}.check span{color:var(--green);font-weight:900;margin-right:8px}.section{margin-top:36px}code{display:block;margin-top:8px;padding:10px;border-radius:12px;background:#06101a;color:#d8f7ff;border:1px solid rgba(79,209,255,.18);overflow:auto}@media(max-width:920px){.grid{grid-template-columns:1fr}.top{align-items:flex-start;flex-direction:column}}
  </style>
</head>
<body>
  ...
  <main class="wrap">
    ...
    <section class="section">
      ...
      <div class="checklist">
        <div class="check"><span>✓</span> Node routes: /, /omos, /protocol, /algorithm, /alignment-api, /tools, /docs, /plugin-bridge</div>
