...
final class OMOS_Admin {
    ...
    public function __construct(OMOS_Runtime_Client $client, OMOS_Shortcodes $shortcodes, $site_role_callback) {
        $this->client = $client;
    ...
    public function bridge() {
        $body = '<div class="omos-core-panel"><h2>Authority Boundary</h2><p><strong>OMOS.OneGodian.org</strong> is the WordPress public/presentation surface. <strong>OMOS.OneGodian.com</strong> is the governed Node/Express runtime. WordPress does not own provider credentials, Council authority, Human Gate decisions, Decision Records, PostgreSQL, deployments, payments, or consequential external actions.</p><p>ACC remains the control plane for separately authorized external execution.</p></div><div class="omos-core-panel"><h2>Verification Endpoints</h2><code>' . esc_html(rest_url('omos/v1/status')) . '</code><br><code>' . esc_html(rest_url('omos/v1/sync')) . '</code><br><code>' . esc_html(rest_url('omos/v1/shortcodes')) . '</code></div>';
    ...
    public function checklist() {
        ...
        $body .= '</ul><p><strong>Repository checks do not certify a live installation.</strong> Use WPVibe to verify the installed plugin version and the <code>/wp-json/omos/v1/sync</code> response on each target site.</p></div>';
