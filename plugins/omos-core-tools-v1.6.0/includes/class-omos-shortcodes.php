...
final class OMOS_Shortcodes {
    ...
    private $compatibility = array(
        ...
        'omos_member_portal'    => 'render_member_portal',
    );

    public function __construct(OMOS_Runtime_Client $client, OMOS_Members_Bridge $members) {
        $this->client = $client;
    ...
    public function render_unity_dashboard() {
        ...
        $body = '<div class="omos-core-metrics">'
            ...
            . $this->metric('Members / INO', !empty($members['available']) ? ('Connected' . (!empty($members['version']) ? ' v' . $members['version'] : '')) : 'Not detected')
    ...
    public function render_member_portal() {
        ...
        $version = !empty($status['version']) ? ' v' . $status['version'] : '';
        return $this->shell('Member Portal', '<div class="omos-core-status-row"><span class="omos-core-status ' . (!empty($status['available']) ? 'is-connected' : 'is-review') . '">Members / INO Platform' . esc_html($version) . '</span></div>' . $this->members->render_member_portal(), 'omos-member-portal');
    }
}
